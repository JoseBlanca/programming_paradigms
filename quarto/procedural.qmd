...
However, very soon computers became more powerful, programs grew and code became unwieldy thus the [spaghetti code](https://en.wikipedia.org/wiki/Spaghetti_code) was born.
Programmers thought about how to solve the issue and they came with the idea that they had to divide the code into small subtasks. This allows for:
...
They thought about how to create these subtasks, these small pieces of functionallity and they created the idea of the [function](https://en.wikipedia.org/wiki/Function_(computer_programming)).
...
They are somewhat similar to a mathematical function, they take some inputs, that could be some numbers of any other kind of data, they do some calculations, some transformations, and they might return a result.
...
There are [typing tools](https://typing.readthedocs.io/), like [mypy](https://mypy-lang.org/) that can be used to enforce them, but they are not part of the language proper.
...
The program is a complex machine build with small, and hopefully understandable, lego pieces, and the interface is the shape of the piece.

We could split the interface in two: the explicit or interface proper, and the implicit interface.
...
The implicit interface is comprised by the interactions of the function with the rest of the program that are not explicitly declared like:

- any modification state of the objects passed to the function that are not clearly stated and, thus, not expected by the user.
...
The interface proper is the stated contract that the function explicitely declares to follow and its at the base of the software modularity. While the implicit interface is, although it can be useful, and even completely necesary in some circunstances, is a modularity violation and a throw back to the spaghetti code, to an unmaintainable mess.

### Name and namespace

Of course, the function name is a critical part of the function interface, if we change it the rest of the code that uses that function will have to be changed.
But also the namespace, where the function is located is important.
...
The arguments accepted by the function, as well as its types, are also part of the interface, if we were to change the arguments required or accepted we might change the rest of the code, the parts of the program that make calls to the modified function.

### Value change

We can pass data to a function if two different ways:
...
For inmutable types this is not a problem at all because even if different variables hold references to the same object, none of them could change the value, but with mutable data types we have a problem. For instance, a function could change a list, that has been passed to it by a caller, by appending values to it without the caller awarenes.
So, in Python mutable objects could be altered inside any function and that is part of the interface, but a part that could not be explicit.
...
A more suttle part of the interface, one that is usually not explicit, is a change in the state of an object.
This is known as a [side effect](https://en.wikipedia.org/wiki/Side_effect_(computer_science)), an effect other than reading the value of the function arguments and returning the intended result. 

For instance, imagine that you are reading from a file and that you pass the file object to a function, that function could change the state of the object, for instance by consuming from the file or by closing it.
...
After calling to the different functions the behaviour of the program will be quite different, so the change in the state of the file object is part of the function interface.
It is always a good policy to informe to the caller/user of the function about these changes of state by making them explicit.

Another kind of side effect to affect the global scope.
This is highly discuraged, there is a global scope and many times it is reasonable to use it to read, for instance, inmutable configuration variables. But altering a variable in the global state takes us back to the path of unmaintanable spaghetti code.

In Python the global scope is frequently used to store configuration constants. (By the way, if you follow the [PEP8](https://peps.python.org/pep-0008/) style guide, as you should, contants defined in the global scope are written with all capital letters.)
...
However, in general, try not to abuse of the global scope. Functions were desing to modify only its scope and having side effects goes agaist the modularity achived by having functions in the first place.
...
The global statement is a nice remainder of the problem of altering the global state, but in Python we can alter the global state, the variables that belong to the global scope, without using the global statement.
...
There is another mechanism to store states between funcion calls that respects the modularity: create a class and instantiate an object. Objects are modular and they allow us to store states between calls. That is one of the big difference between the procedural and the object oriented approach.

### Returned values

It is also fairly obvious that the returned values and types are also part of the interface, if we change what we return we will have to alter the calls to the function.
...
This is a well defined interface, but the caller will have to test what is getting from the function.
...
Now the caller explicitly aware of the two possible types and will need to decide to deal the situation. Before the problem was not explicit and the unaware caller could have a program that worked in some ocasions and failed in some other times without being aware of the problem.
...
In that way the caller we'll be aware that a problem happened only when the problem arises, and will have to deal the the error only when needs to, and not in every call in the [call stack](https://en.wikipedia.org/wiki/Call_stack).
