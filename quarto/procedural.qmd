...
width = 10
height = 4
print_area(calc_rectangle_area(width, height))
```
...
As we have seen, we **access** data stored in memory by using **variables**.
We could think that, idealy, those variables, once they created, should be **globally available** in every part of our program.
...
We say that functions define a **scope, an area of the program in which certain variables can be accessed**.
...
For instance, variables that are defined inside a function are **not available outside** the function.
...
For instance, if we store an object in memory, like the value 2, that object does not belong to a scope, the variables that point to it do.
...
In general, try not to abuse of the global scope. Functions were desinged to modify only its scope and having **side effects goes agaist the modularity** achived by having functions in the first place.
...
```
def calc_rect_area(width:float, height:float) -> float:
    area = width * height
    return area
```
...
Remember the [Zen](https://peps.python.org/pep-0020/): explicit is better than implicit.
...
In general, we could pass data to a function in two different ways by:
